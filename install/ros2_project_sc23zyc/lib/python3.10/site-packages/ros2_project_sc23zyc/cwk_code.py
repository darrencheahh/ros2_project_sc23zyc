...
class ColourNavigator(Node):
    ...
    def spin_and_detect(self):
        ...
        while rclpy.ok():
            ...
            if self.latest_image is not None:
                self.get_logger().info("📸 Camera feed ready.")
                break

            if time.time() - start_time > 5.0:
                self.get_logger().warn("⚠️ Camera feed not ready after 5 seconds.")
                break
    ...
    def center_and_approach_blue(self):
        ...
        while rclpy.ok() and time.time() - start_time < timeout:            
            ...
            if self.front_distance > 2.0:
                ...
                forward.linear.x = 0.06
                
            self.cmd_vel_pub.publish(forward)
            
            self.get_logger().info(f"🚗 Approaching... front_distance = {self.front_distance:.2f} m")
    ...
    def send_next_goal(self):
        ...
        self.get_logger().info(f"hehe Navigating to corner {self.goal_index}: (x={x:.1f}, y={y:.1f})")
